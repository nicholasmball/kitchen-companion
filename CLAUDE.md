...
- **Pause / Push back / Padding (interruption-tolerant timing):** sticky Pause + Push-back bar at the top of the planner page (56 px tap targets — "messy hands" principle). **Pause = freeze, no schedule change**: amber banner with elapsed counter, "Don't forget the oven is still on!" hint, future notifications cancelled, displayed timeline frozen (clock-times and countdowns stop ticking — only the elapsed counter advances). **Resume** clears `paused_at` and that's it — schedule is unchanged. If a scheduled event passed during the pause window, a Sonner toast offers a one-tap push-back catch-up; cook decides. **Push-back** is the explicit re-plan path (+5/+10/+15/+30 quick picks plus custom datetime), and is in-flight aware (items already cooking keep their physical finish time via `cook_end_override`). Plan create/edit form has a "Buffer minutes" field (default 0, max 120) that adds slack at the start so small interruptions don't cascade. All sync is manual; in-flight cooks never auto-mutated. See `lib/plan-pause.ts` for the time-arithmetic library + `docs/planner-pause-design.html` and `docs/pause-redesign-mockup.html` for the UX spec.
- **Push-back respects items already cooking:** when push-back is invoked while items are mid-cook (cook_start time in the past), those items' cook_end is anchored at the original time via `meal_items.cook_end_override`. Their physical finish time stays put; only the serve event and not-yet-started items shift. If items finish before the new serve time, the push-back toast surfaces a "keep warm" hint. See `lib/in-flight.ts`.
...
- **Recipe ↔ planner sync:** meal items are snapshotted from recipes (`meal_items.recipe_snapshot_at` timestamp). When the source recipe is edited later, the planner shows an amber "Recipe updated" badge on affected meal items; tapping it opens a diff dialog with a Refresh button. After the cook edits a meal item (in the planner) that diverges from the source recipe, a non-blocking toast offers "Save to recipe?" — opens a per-change opt-in dialog (Step/Method changes default ON; time tweaks default OFF as likely tonight-only). Conflict path (recipe also edited externally) offers either "pull recipe first" or "replace recipe with my version". The recipes list shows "N plan items stale" on each recipe with stale snapshots. Settings has an "Offer to save planner edits back to recipes" toggle (default ON, persisted in localStorage). All sync is manual — never auto. See `lib/recipe-sync.ts` for the diff library + `docs/recipe-plan-sync-design.html` for the UX spec.
